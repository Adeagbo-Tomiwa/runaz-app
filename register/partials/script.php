...
(function(){
  ...
  const showStep = (n)=>{
    ...
    $$('#steps .step').forEach((li,i)=>{
      if(i < step-1){
        ...
        div?.classList.remove('ring-2', 'ring-runaz-blue');
      } else if(i === step-1){
        // Current step
        li.classList.add('opacity-100');
        li.classList.remove('opacity-60');
  ...
  const validateStep = ()=>{
    ...
    if(step === 4){
      ...
      if(selectedRole === 'runner'){
        // Check that at least one service category is selected - FIXED SELECTOR
        const checkedCategories = form.querySelectorAll('input[name="categories[]"]:checked');
        
        console.log('Checking categories:', checkedCategories.length); // Debug
        
        if(!checkedCategories || checkedCategories.length === 0){
          showToast('Please select at least one service category', 'error');
          // Scroll to categories section
          const categoriesSection = form.querySelector('input[name="categories[]"]')?.closest('.sm\\:col-span-2');
          categoriesSection?.scrollIntoView({ behavior: 'smooth', block: 'center' });
          return false;
        }

        // Validate skills
        if(!form.skills || !form.skills.value.trim()){
          showToast('Please enter your skills or keywords', 'error');
          form.skills?.focus();
          return false;
        }

        // Validate hourly rate
        if(!form.rate || !form.rate.value || Number(form.rate.value) <= 0){
        ...
        // Validate experience
        if(!form.experience || !form.experience.value || Number(form.experience.value) < 0){
          showToast('Please enter your experience in years', 'error');
          form.experience?.focus();
          return false;
        }
      } else if(selectedRole === 'requester'){
        // Validate requester-specific fields
        if(!form.default_address || !form.default_address.value.trim()){
          showToast('Please enter your default service address', 'error');
          form.default_address?.focus();
          return false;
        }
      }
    }
  ...
  const buildReview = ()=>{
    ...
    if(form.alt_phone && form.alt_phone.value){
      pairs.push(['Alternate Phone', form.alt_phone.value]);
    }
    
    if(form.referral && form.referral.value){
      pairs.push(['Referral', form.referral.value]);
    }
    
    if(role === 'runner'){
      // Get selected category names - FIXED SELECTOR
      const cats = form.querySelectorAll('input[name="categories[]"]:checked');
      const catNames = Array.from(cats).map(cb => {
        const label = cb.closest('label');
        // Get text content but remove checkbox element text
        const labelText = label ? label.textContent.trim() : cb.getAttribute('data-category-name') || cb.value;
        return labelText;
      });
      pairs.push(['Service Categories', catNames.join(', ') || '—']);
      
      if(form.skills) pairs.push(['Skills', form.skills.value || '—']);
      if(form.rate) pairs.push(['Hourly Rate', form.rate.value ? `₦${form.rate.value}/hr` : '—']);
      if(form.experience) pairs.push(['Experience', form.experience.value ? `${form.experience.value} years` : '—']);
      if(form.bio && form.bio.value) pairs.push(['Bio', form.bio.value.substring(0,100) + (form.bio.value.length>100?'...':'')]);
      if(form.availability && form.availability.value) pairs.push(['Availability', form.availability.value]);
    } else {
      if(form.default_address) pairs.push(['Service Address', form.default_address.value||'—']);
      if(form.prefer_verified) pairs.push(['Prefer Verified', form.prefer_verified.value||'Yes']);
      if(form.budget_pref) pairs.push(['Budget Preference', form.budget_pref.value||'Flexible']);
      if(form.notes && form.notes.value) pairs.push(['Notes', form.notes.value.substring(0,100) + (form.notes.value.length>100?'...':'')]);
    }
    
    // Files uploaded confirmation
    const filesUploaded = [];
    if(form.selfie && form.selfie.files[0]) filesUploaded.push('Selfie');
    if(form.id_front && form.id_front.files[0]) filesUploaded.push('ID Front');
    if(form.id_back && form.id_back.files[0]) filesUploaded.push('ID Back');
